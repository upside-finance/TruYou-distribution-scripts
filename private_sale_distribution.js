...
const main = async () => {
  ...
  const calcTokenDistributionForMonth = (totalTokens, month) => {
    ...
    if (month == 1) {
      ...
      alreadyDistributed = totalTokens * 0.06 * (month - 1);
    }

    if (alreadyDistributed >= totalTokens) {
      return 0;
    } else {
      const toBeDistributed = totalTokens * 0.06;
  ...
  const genOutputCSV = async (distributionArr, fileName) => {
    ...
    await csv.toDisk(`./output/private_sale/${fileName}_month_${month}.csv`, {
      append: true,
    });
  };
  ...
  const csv = load(
    "distribution_schedule/Private Sale Allotments and Allocations.csv"
  );
  for (const index in csv) {
    const row = csv[index];

    const address = row["Private Sale @ 0.002 Beginning 04/23"];
    const totalTokens = parseFloat(row["Total token allotment"]);
